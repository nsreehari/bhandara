...
#openerp_background_worker_code_start
OPENERP_BACKGROUND_WORKER_SCRIPT=/opt/bitnami/apps/odoo/scripts/openerp_background_worker.sh
#openerp_background_worker_code_end
#openerp_gevent_code_start
OPENERP_GEVENT_SCRIPT=/opt/bitnami/apps/odoo/scripts/openerp_gevent.sh
#openerp_gevent_code_end
...
OPENERP_RESTFUL_SCRIPT=/home/bitnami1/restful-odoo-proxy/restful_ctl.sh
#OPENERP_zESTFUL_SCRIPT=/home/bitnami1/whatsapp/whatsapp_ctl.sh
...
help() {
	...
	if test -x $LIBREOFFICE_SCRIPT; then	
        ...
        #openerp_background_worker_code_start
        if test -x $OPENERP_BACKGROUND_WORKER_SCRIPT; then  
            echo "       $0 (start|stop|restart|status) openerp_background_worker"
        fi
        #openerp_background_worker_code_end
        #openerp_gevent_code_start
        if test -x $OPENERP_GEVENT_SCRIPT; then  
            echo "       $0 (start|stop|restart|status) openerp_gevent"
        fi
        #openerp_gevent_code_end
        #openerp_zestful_code_start
        if test -x $OPENERP_zESTFUL_SCRIPT; then  
            echo "       $0 (start|stop|restart|status) whatsapp"
        fi
        #openerp_zestful_code_end
        #openerp_restful_code_start
        if test -x $OPENERP_RESTFUL_SCRIPT; then  
            echo "       $0 (start|stop|restart|status) openerp_restful"
        fi
        #openerp_restful_code_end
...
if [ "x$1" = "xhelp" ] || [ "x$1" = "x" ]; then
    ...
    #openerp_background_worker_code_start
        elif [ "x$2" = "xopenerp_background_worker" ]; then
        if test -x $OPENERP_BACKGROUND_WORKER_SCRIPT; then  
            $OPENERP_BACKGROUND_WORKER_SCRIPT start  
            OPENERP_BACKGROUND_WORKER_ERROR=$?
        fi
        #openerp_background_worker_code_end
        #openerp_gevent_code_start
        elif [ "x$2" = "xopenerp_gevent" ]; then
        if test -x $OPENERP_GEVENT_SCRIPT; then  
            $OPENERP_GEVENT_SCRIPT start  
            OPENERP_GEVENT_ERROR=$?
        fi
        #openerp_zestful_code_start
        elif [ "x$2" = "xwhatsapp" ]; then
        if test -x $OPENERP_zESTFUL_SCRIPT; then  
            $OPENERP_zESTFUL_SCRIPT start  
            OPENERP_zESTFUL_ERROR=$?
        fi
        #openerp_zestful_code_end
        #openerp_gevent_code_end
        #openerp_restful_code_start
        elif [ "x$2" = "xopenerp_restful" ]; then
        if test -x $OPENERP_RESTFUL_SCRIPT; then  
            $OPENERP_RESTFUL_SCRIPT start  
            OPENERP_RESTFUL_ERROR=$?
        fi
        #openerp_restful_code_end
    ...
    #openerp_background_worker_code_start
        ...
        if test -x $OPENERP_BACKGROUND_WORKER_SCRIPT; then
            $OPENERP_BACKGROUND_WORKER_SCRIPT stop
            OPENERP_BACKGROUND_WORKER_ERROR=$?
        fi
        #openerp_background_worker_code_end
        #openerp_gevent_code_start
    ...
    #openerp_background_worker_code_start
        ...
        if test -x $OPENERP_BACKGROUND_WORKER_SCRIPT; then
            $OPENERP_BACKGROUND_WORKER_SCRIPT status
        fi
        #openerp_background_worker_code_end
        #openerp_gevent_code_start
        elif [ "x$2" = "xopenerp_gevent" ]; then
        if test -x $OPENERP_GEVENT_SCRIPT; then
            $OPENERP_GEVENT_SCRIPT status
        fi
        #openerp_gevent_code_end
        #openerp_zestful_code_start
        elif [ "x$2" = "xwhatsapp" ]; then
        if test -x $OPENERP_zESTFUL_SCRIPT; then
            $OPENERP_zESTFUL_SCRIPT status
        fi
        #openerp_zestful_code_end
        #openerp_restful_code_start
        elif [ "x$2" = "xopenerp_restful" ]; then
        if test -x $OPENERP_RESTFUL_SCRIPT; then
            $OPENERP_RESTFUL_SCRIPT status
        fi
        #openerp_restful_code_end
