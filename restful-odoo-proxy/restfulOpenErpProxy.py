...
# (C) 2012, 2013 Tobias G. Pfeiffer <tgpfeiffer@web.de>
...
def requestwrite1(request, str):
    request.write(str)

def requestSetCrossDomainHdrs(request):
...
def requestwrite(request, str):
    hello()
    requestSetCrossDomainHdrs(request)
    if 'callback' in request.args.keys():
        wrapper = request.args['callback'][0]
        requestwrite1(request, '%s(%s)' % (wrapper, str))
    else:
        requestwrite1(request, str)

default_cred_admin = 'admin'
default_cred_pwd = 'bitnami'

def requestGetUser(request):
    if default_cred_admin:
            return default_cred_admin
    return request.getUser()

def requestGetPassword(request):
    if default_cred_pwd:
            return default_cred_pwd
...
def localTimeStringToUtcDatetime(s):
    ...
    t = datetime.datetime.strptime(s, '%Y-%m-%d %H:%M:%S')  # this time is in local tz
...
class OpenErpDispatcher(Resource, object):
    ...
    def getChildWithDefault(self, pathElement, request):
        ...
        if not (requestGetUser(request) and requestGetPassword(request)):
...
class OpenErpModelResource(Resource):
    ...
    def __getCollection(self, uid, request, pwd):
        ...
        for key, vals in request.args.iteritems():
            if key in IGNORE_KEYS:
                continue
            if not key in self.desc and key != "id":
    ...
    def __handleCollectionAnswer(self, val, request, uid, pwd):
        ...
        def createFeed(items, request):
            ...
            requestwrite(request, str(feed.to_string().encode('utf-8')))
            request.finish()
    ...
    def __getCollectionExpandFields(self, uid, request, pwd, expandfields):
        """This is called after successful login to list the items
        of a certain collection, e.g. all res.partners."""
        hello()
        params = []
        ...
        for key, vals in request.args.iteritems():
            ...
            if key == "xml":
                ...
                if len(vals) > 0:
                    for vv in vals:
                      for v in vv.split(','):
                        if v and v not in self.desc:
                            raise InvalidParameter("field '%s' not present in model '%s'" % (v, self.model))

                        if v not in summaryCols:
                            summaryCols.append(v)




            elif not key in self.desc and key != "id":
                raise InvalidParameter("field '%s' not present in model '%s'" % (key, self.model))
            else:
                if len(vals) == 1:
                    try:
        ...
        proxy = quietProxy(self.openerpUrl + 'object')
        d = proxy.callRemote('execute', self.dbname, uid, pwd, self.model, 'search', params)
        d.addCallback(self.__handleCollectionAnswerExpandFields, request, uid, pwd, summaryCols, formatJson)
        return d

    def __handleCollectionAnswerExpandFields(self, val, request, uid, pwd, summaryCols, formatJson):
        hello()
        ...
        def createFeed(items, request):
            ...
            if formatJson:
                ...
                ritems = self.__mkSummaryJSON(str(request.URLPath()), self.desc, summaryCols, items)
                str1 = json.dumps(ritems)
                request.setHeader("Content-Type", "application/json; charset=utf-8")

                requestwrite(request, str(str1.encode('utf-8')))
                request.finish()
                ...
                for item in items:
                    ...
                    summaryStr = ""
                    if summaryCols:
                        summaryStr = self.__mkSummaryXml(str(request.URLPath()), self.desc, summaryCols, item)

                    #for s in summaryCols:
                    #   summaryStr += ' --  %s:%s' % (s, item[s])
        ...
        fieldList = ['name', '__last_update']
        if not formatJson:
            fieldList += ['user_id']
        fieldList += summaryCols

        d = proxy.callRemote('execute', self.dbname, uid, pwd, self.model, 'read', val, fieldList)
        d.addCallback(createFeed, request)
        return d
    ...
    def __getItemDefaults(self, uid, request, pwd):
        ...
        requestwrite(request, self.__mkDefaultXml(str(request.URLPath()), self.desc, self.defaults[uid]))
        request.finish()
...
''' % (self.model,
        ...
        for key in desc.iterkeys():
            value = key in item and item[key] or ""
    ...
    def __mkSummaryJSON(self, path, desc, desckeys, items):
        ...
        for item in items:
          ...
          for key in desckeys:
            ...
            # display URIs for many2one fields
                ...
                item['related.' + key] =  {
                        'id':str(value[0]),
                        'href': '/'.join(path.split("/")[:-1] + [self.desc[key]["relation"], str(value[0])]),
                        'name': value[1]
                }
            ...
            # display URIs for *2many fields, wrapped by <item>
                ...
                item['related.' + key] =  [{
                    'id': str(v),
                    'href': '/'.join(path.split("/")[:-1] + [self.desc[key]["relation"], str(v)])
                    } for v in value]

            # for other fields, just output the data
            elif fieldtype == 'boolean':
                item[key] = value and True or False
    ...
    def __mkSummaryXml(self, path, desc, desckeys, item):
        ns = "".join([word[0] for word in self.model.split('.')])
        xml = '''
    <%s xmlns:%s="%s">
        <%s:id />
        ...
        for key in desckeys:
            ...
            # display URIs for many2one fields
                ...
                xml += ("    <%s type='%s' relation='%s'>\n      <link href='%s' />\n    </%s>\n" % (
                    ...
                    fieldtype,
                    '/'.join(path.split("/")[:-1] + [self.desc[key]["relation"]]),
                    '/'.join(path.split("/")[:-1] + [self.desc[key]["relation"], str(value[0])]),
                    ns + ":" + key)
                )
            # display URIs for *2many fields, wrapped by <item>
            elif fieldtype in ('one2many', 'many2many'):
                ...
                if len(vals) > 0:
                    for vv in vals:
                      for v in vv.split(','):
                        ...
                        if v not in fieldList:
                            fieldList.append(v)


        # issue the request
        proxy = quietProxy(self.openerpUrl + 'object')
        d = proxy.callRemote('execute', self.dbname, uid, pwd, self.model, 'read', [modelId], fieldList, params)


        if 'xml' in params.keys():
            d.addCallback(self.__handleItemAnswer, request, localTimeStringToUtcDatetime(updateTime))
        else:
            d.addCallback(self.__handleItemAnswerExtraFields, request, localTimeStringToUtcDatetime(updateTime), True, fieldList, buttonsRequired)
        return d

    def __mkItemJSON(self, basePath, path, lastModified, item, descKeys, btnRequired):
        result = ""
        ritem = {}
        # loop over the fields of the current object
        ...
        for key in descKeys:
              ...
              # display URIs for *2many fields, wrapped by <item>
                  ...
                  ritem[key] = [str(v) for v in value]

              # for other fields, just output the data
              elif fieldtype == 'boolean':
                  ritem[key] = value and True or False
        ...
        if btnRequired:
          ...
          for button in self.workflowDesc:
            if "name" in button.attrib and \
                    (not "state" in item or not "states" in button.attrib or item["state"] in button.attrib['states'].split(",")) \
                    and not self.__is_number(button.attrib["name"]):
                buttons.append({
                        'rel': button.attrib['name'],
                        'href': path + "/" + button.attrib['name'],
                        'title': button.attrib['string']
                        })
          ritem['buttons'] = buttons
        return json.dumps(ritem)

    def __handleItemAnswerExtraFields(self, val, request, lastModified, formatJSON, fieldList, buttonsRequired):
        hello()
        ...
        try:
            ...
            requestwrite(request, "No such resource.")
            request.finish()
            return
        ...
        if formatJSON:
            ...
            if fieldList:
                s = self.__mkItemJSON(basepath, path, lastModified, item, fieldList, buttonsRequired)
            else:
                s = self.__mkItemJSON(basepath, path, lastModified, item, self.desc, True)

            requestwrite(request, s)
            request.finish()
...
<entry xmlns="http://www.w3.org/2005/Atom">
    ...
    <content type="application/vnd.openerp+xml">
    <%s xmlns:%s="%s">
''' % (xmlescape(unicode('name' in item and item['name'] or "None")),
             path,
    ...
    ### handle JSON inserts into collection
    ...
    def __addToCollection(self, uid, request, pwd):
        ...
        try:
            ...
            requestwrite(request, " 3 malformed JSON: " + str(e))
            request.finish()
            return

        for k in ['id', 'create_date']:
            if  k in ritem.keys():
                del ritem[k]
        ...
        for tagname in ritem.keys():
            ctext = ritem[tagname]
            if tagname in self.desc.keys():
                desc = self.desc[tagname]

            if desc["type"] in ("char", "selection", "text", "datetime"):
                fields[tagname] = ctext
            elif desc["type"] == "float":
                fields[tagname] = float(ctext)
            elif desc["type"] == "integer":
                fields[tagname] = int(ctext)
            elif desc["type"] == "boolean":
                fields[tagname] = (ctext == "True")
            elif desc["type"] == "many2one":
                fields[tagname] = int(ctext)
            elif desc["type"] in ("many2many", "one2many"):
                pass
            else:
                # TODO: date, many2one (we can't really set many2many and one2many here, can we?)
                raise NotImplementedError("don't know how to handle element " + tagname + " of type " + desc["type"])
    ...
    def __addToCollectionXML(self, uid, request, pwd):
        """This is called after successful login to add an items
        to a certain collection, e.g. a new res.partner."""
        hello()
        if not self.desc:
        ...
        try:
            ...
            requestwrite(request, " 4 malformed XML: " + str(e))
            request.finish()
            return
        ...
        if not relaxng.validate(doc):
            ...
            requestwrite(request, "invalid XML:\n" + str(err))
            request.finish()
            return
    ...
    def __executeWorkflow(self, val, uid, request, pwd, modelId, workflow):
        ...
        for button in self.workflowDesc:
            if "name" in button.attrib and \
                    (not "state" in item or item["state"] in button.attrib['states'].split(",")) \
            ...
            requestwrite(request, "Workflow '%s' not allowed in state '%s'." %
                (workflow, ("state" in item and item["state"]) or ''))
            request.finish()
            return
        # here, the workflow is allowed for the current object
        if "type" in currentAction.attrib and currentAction.attrib['type'] == "object":
            ...
            return d
        elif "type" in currentAction.attrib:
    ...
    def __getItemForDelete(self, (uid, updateTime), request, pwd, modelId):
        hello()
        # make sure we're dealing with an integer id
        try:
            modelId = int(modelId)
        ...
        proxy = quietProxy(self.openerpUrl + 'object')
        d = proxy.callRemote('execute', self.dbname, uid, pwd, self.model, 'read', [modelId], [], [])
        d.addCallback(self.__deleteItem, uid, pwd, request, localTimeStringToUtcDatetime(updateTime))
        return d

    def __deleteItem(self, old, uid, pwd, request, lastModified):
        """This is called after successful login to add an items
        to a certain collection, e.g. a new res.partner."""
        hello()
        if not self.desc:
        ...
        # compose the XML-RPC call from them
        proxy = quietProxy(self.openerpUrl + 'object')
        d = proxy.callRemote('execute', self.dbname, uid, pwd, self.model, 'unlink', [old[0]['id']] )
    ...
    def __updateItem(self, old, uid, pwd, request, lastModified):
        ...
        try:
            ritem = json.loads(request.content.read())
        except Exception as e:
            request.setResponseCode(400)

            requestwrite(request, " 1 malformed JSON: " + str(e))
            request.finish()
            return
    ...
    def __updateItemXML(self, old, uid, pwd, request, lastModified):
        """This is called after successful login to add an items
        to a certain collection, e.g. a new res.partner."""
        hello()
        if not self.desc:
        ...
        try:
            ...
            requestwrite(request, " 2 malformed XML: " + str(e))
            request.finish()
            return
        # check whether we got valid XML with the given schema
        ns = str(request.URLPath()) + "/schema"
...
''' % (self.model.replace(".", "_"), ns)
        for key, val in desc.iteritems():
            fieldtype = val['type']
            required = "required" in val and val['required'] or False
    ...
    def __getSchema(self, uid, request):
        ...
        if not self.desc:
            ...
            requestwrite(request, "Schema description not found")
            request.finish()
            return
        else:

            requestwrite(request, self.__desc2relaxNG(str(request.URLPath()), self.desc))
            request.finish()
    ...
    def __cleanup(self, err, request):
        ...
        if err.check(xmlrpclib.Fault):
            if e.faultCode == "AccessDenied":
                ...
                requestwrite(request, "Bad credentials.")
                ...
                requestwrite(request, "No such collection.")
            else:
                request.setResponseCode(500)

                requestwrite(request, "An XML-RPC error occured:\n" + e.faultCode.encode("utf-8"))
        elif e.__class__ in (InvalidParameter, PostNotPossible, PutNotPossible, NoChildResources, NotFound):
            request.setResponseCode(e.code)

            requestwrite(request, str(e))
        else:
            request.setResponseCode(500)

            requestwrite(request, "An error occured:\n" + str(e))
        request.finish()
...
It only throws the given exception."""
    ...
    def render_GET(self, request):
        hello()
        user = requestGetUser(request)
        pwd = requestGetPassword(request)
        ...
        if not request.postpath:
            d.addCallback(self.__getCollectionExpandFields, request, pwd, 1)

        # if URI is sth. like /[dbname]/res.partner/collection,
        #  list this particular schema
        elif len(request.postpath) == 1 and request.postpath[0] == "original":
    ...
    def render_DELETE(self, request):
        ...
        if len(request.postpath) == 1 and self.__is_number(request.postpath[0]):
            d.addCallback(self.__getLastItemUpdate, request, pwd, request.postpath[0])
            d.addCallback(self.__getItemForDelete, request, pwd, request.postpath[0])

        # if URI looks different, return 400, cannot DELETE here
        else:
            d.addCallback(self.__raiseAnError,
                DeleteNotPossible(str(request.URLPath()) + ''.join(['/' + r for r in request.postpath])))
...
class DeleteNotPossible(Exception):
    code = 400
    ...
    def __str__(self):
        return "You cannot DELETE to " + str(self.res)
